...
from project_models import Annotation, AnnotationTimerSession, Project, Task, TaskAssignment
...
async def create_annotation(
    ...
    # Prevent duplicate auto-submitted annotations for the same task by the same user.
    # This can happen when a user re-enters labeling and the client-side auto-submit
    # fires again for a task that was already auto-submitted.
    if annotation.auto_submitted:
        existing_auto = (
            db.query(Annotation)
            .filter(
                Annotation.task_id == task_id,
                Annotation.completed_by == current_user.id,
                Annotation.auto_submitted == True,
            )
            .first()
        )
        if existing_auto:
    ...
    # Server-side lead_time from timer session (Issue #1205)
    server_lead_time = annotation.lead_time
    auto_submitted = annotation.auto_submitted or False
    timer_session = (
        db.query(AnnotationTimerSession)
        .filter(
            AnnotationTimerSession.task_id == task_id,
            AnnotationTimerSession.user_id == current_user.id,
            AnnotationTimerSession.completed_at == None,
        )
        .first()
    )
    if timer_session:
        now = datetime.now(timezone.utc)
        server_lead_time = (now - timer_session.started_at).total_seconds()
        timer_session.completed_at = now
        timer_session.auto_submitted = auto_submitted
