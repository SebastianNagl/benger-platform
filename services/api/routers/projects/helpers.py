...
def get_comprehensive_project_data(db: Session, project_id: str) -> Dict[str, Any]:
    ...
    project_data = {
        ...
        # Feedback settings
        "feedback_enabled": project.feedback_enabled,
        "feedback_config": project.feedback_config,
    }
    ...
    from routers.projects.serializers import serialize_feedback_comment

    feedback_comments = (
        db.query(FeedbackComment)
        .filter(FeedbackComment.project_id == project_id)
        .order_by(FeedbackComment.created_at.asc())
        .all()
    )
    feedback_comments_data = [
        serialize_feedback_comment(fc, mode="full") for fc in feedback_comments
    ]
    ...
    for annotation in annotations:
        ...
        if annotation.reviewed_by:
            user_ids.add(annotation.reviewed_by)
    for member in project_members:
        if member.user_id:
    ...
    for fc in feedback_comments:
        if fc.created_by:
            user_ids.add(fc.created_by)
        if fc.resolved_by:
            user_ids.add(fc.resolved_by)
    ...
    statistics = {
        ...
        "total_feedback_comments": len(feedback_comments_data),
    }
    ...
    return {
        ...
        # Feedback data
        "feedback_comments": feedback_comments_data,
        # Statistics
