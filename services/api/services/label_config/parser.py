...
class LabelConfigParser:
    ...
    SUPPORTED_FIELD_TYPES = [
        ...
        "Text",
        "DateTime",
        "Gliederung",  # German legal outline (Issue #1041)
        "Loesung",  # German legal solution (Issue #1041)
        "Angabe",  # Read-only markdown with highlights and comments
        "Notizen",  # Free-form markdown notes (Issue #1041)
    ]
    ...
    def _parse_field_element(elem: ET.Element) -> Optional[Dict]:
        ...
        elif elem.tag == "Angabe":
            field["labels"] = LabelConfigParser._extract_angabe_labels(elem)
    ...
    @staticmethod
    def _extract_angabe_labels(angabe_elem: ET.Element) -> List[Dict]:
        """
        Extract highlight labels from an Angabe element.
        Uses the same <Label> child pattern as <Labels>.

        Args:
            angabe_elem: Angabe XML element

        Returns:
            List of label dictionaries with value and background
        """
        labels = []
        for label in angabe_elem.findall("Label"):
            value = label.get("value")
            if value:
                labels.append(
                    {
                        "value": value,
                        "background": label.get("background", "#fef08a"),
                    }
                )
        return labels
