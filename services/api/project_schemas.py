...
class ProjectUpdate(BaseModel):
    ...
    immediate_evaluation_enabled: Optional[bool] = Field(
        None, description="Show evaluation results modal after annotation submission (Issue #998)"
    )
    # Annotation time limit (Issue #1043)
    annotation_time_limit_enabled: Optional[bool] = Field(
        None, description="Enable time limit for annotations"
    )
    annotation_time_limit_seconds: Optional[int] = Field(
        None, ge=60, le=21600, description="Time limit in seconds (1-360 minutes)"
    )
    # Strict timer mode (Issue #1205)
    strict_timer_enabled: Optional[bool] = Field(
        None, description="Enable strict timer mode (requires annotation_time_limit_enabled)"
    )
    ...
    # Review settings
    review_enabled: Optional[bool] = Field(None, description="Enable review stage for this project")
    review_mode: Optional[str] = Field(
        None, description="Review mode: in_place, independent, or both"
    )
    allow_self_review: Optional[bool] = Field(
        None, description="Allow reviewers to review their own annotations"
    )

    # Feedback settings
    feedback_enabled: Optional[bool] = Field(None, description="Enable feedback workflow")
    feedback_config: Optional[List[Dict[str, Any]]] = Field(
        None, description="Feedback highlight label config [{value, background}]"
    )
    ...
    def validate_strict_timer_requires_time_limit(self):
        """Strict timer mode requires annotation_time_limit_enabled."""
        if self.strict_timer_enabled is True and self.annotation_time_limit_enabled is False:
            raise ValueError(
                "strict_timer_enabled requires annotation_time_limit_enabled to be true"
            )
        return self
...
class AnnotationBase(BaseModel):
    ...
    auto_submitted: Optional[bool] = Field(
        False, description="Whether this was auto-submitted by timer expiry (Issue #1205)"
    )
...
class ReviewSubmit(BaseModel):
    ...
    result: Optional[List[Dict[str, Any]]] = Field(
        None, description="Updated/independent annotation result (for fix and independent)"
    )
...
class FeedbackCommentCreate(BaseModel):
    ...
    highlight_start: Optional[int] = Field(None, description="Highlight start offset in source text")
    highlight_end: Optional[int] = Field(None, description="Highlight end offset in source text")
    highlight_text: Optional[str] = Field(None, description="Selected text content")
    highlight_label: Optional[str] = Field(None, description="Highlight label from feedback_config")
...
class ComprehensiveProjectExport(BaseModel):
    ...
    # Feedback data
    feedback_comments: List[Dict[str, Any]] = Field(default=[], description="Feedback comments")
