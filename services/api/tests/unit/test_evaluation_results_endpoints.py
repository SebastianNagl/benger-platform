...
class TestGetTaskPreview:
    """Tests for _get_task_preview helper."""
    ...
    def test_question_key(self):
        assert _get_task_preview({"question": "What is?"}) == "What is?"

    def test_prompt_key(self):
        assert _get_task_preview({"prompt": "A prompt"}) == "A prompt"

    def test_content_key(self):
        assert _get_task_preview({"content": "Content here"}) == "Content here"
    ...
    def test_priority_order_input_first(self):
        data = {"input": "first", "text": "second", "question": "third"}
        assert _get_task_preview(data) == "first"

    def test_fallback_to_string_value(self):
        data = {"custom_field": "fallback value", "number_field": 42}
        result = _get_task_preview(data)
        assert result == "fallback value"

    def test_no_string_values_returns_empty(self):
...
class TestBuildAllTasksResponse:
    ...
    @patch("routers.evaluations.results._get_task_preview")
    def test_basic_response(self, mock_preview, mock_availability):
        mock_preview.return_value = "preview text"
        mock_availability.return_value = ({"task-1"}, {"task-1": {"gpt-4"}})

        db = Mock()
        task1 = Mock(id="task-1", data={"text": "hello"})
        task2 = Mock(id="task-2", data={"text": "world"})
        db.query.return_value.filter.return_value.all.return_value = [task1, task2]

        result = _build_all_tasks_response(db, "proj-1")

        assert len(result) == 2
        assert result[0]["task_id"] == "task-1"
        assert result[0]["has_annotation"] is True
        assert result[0]["generation_models"] == ["gpt-4"]
        assert result[0]["scores"] == {}
        assert result[1]["task_id"] == "task-2"
        assert result[1]["has_annotation"] is False
...
class TestMetricDisplayName:
    """Tests for _metric_display_name."""

    def test_numeric_metric_name(self):
        record = Mock(metrics={"accuracy": 0.95}, field_name="field1")
        assert _metric_display_name(record) == "Accuracy"

    def test_underscored_name_conversion(self):
        record = Mock(metrics={"llm_judge_custom": 0.8}, field_name="field1")
        assert _metric_display_name(record) == "Llm Judge Custom"

    def test_skips_raw_score(self):
        record = Mock(metrics={"raw_score": 0.5, "accuracy": 0.9}, field_name="field1")
        assert _metric_display_name(record) == "Accuracy"

    def test_skips_error_key(self):
        record = Mock(metrics={"error": True, "accuracy": 0.9}, field_name="field1")
        assert _metric_display_name(record) == "Accuracy"

    def test_skips_details_suffix(self):
        record = Mock(
            metrics={"accuracy_details": {"info": "data"}, "accuracy": 0.9},
            field_name="field1",
        )
        assert _metric_display_name(record) == "Accuracy"

    def test_skips_raw_suffix(self):
        record = Mock(
            metrics={"bleu_raw": 0.5, "bleu": 0.8}, field_name="field1"
        )
        assert _metric_display_name(record) == "Bleu"

    def test_skips_grade_points_suffix(self):
        record = Mock(
            metrics={"llm_judge_custom": 12, "llm_judge_custom": 0.8},
            field_name="field1",
        )
        assert _metric_display_name(record) == "Llm Judge Falloesung"

    def test_skips_passed_suffix(self):
        record = Mock(
            metrics={"accuracy_passed": True, "accuracy": 0.7}, field_name="field1"
        )
        assert _metric_display_name(record) == "Accuracy"

    def test_no_numeric_values_uses_field_name(self):
        record = Mock(metrics={"text_key": "hello"}, field_name="my_field")
        assert _metric_display_name(record) == "my_field"

    def test_empty_metrics_uses_field_name(self):
        record = Mock(metrics={}, field_name="fallback")
        assert _metric_display_name(record) == "fallback"

    def test_none_metrics_uses_field_name(self):
        record = Mock(metrics=None, field_name="fallback")
        assert _metric_display_name(record) == "fallback"

    def test_none_field_name_fallback(self):
        record = Mock(metrics=None, field_name=None)
        assert _metric_display_name(record) == "Evaluation"
...
class TestBuildFieldResults:
    """Tests for _build_field_results."""
    ...
    def test_error_record(self):
        ...
        assert results[0].metrics[0].details == {"error": "Timeout"}

    def test_error_record_no_message(self):
        record = Mock(
            metrics={"error": True},
            field_name="field1",
            error_message=None,
    ...
    def test_generic_metrics(self):
        record = Mock(
            metrics={"bleu": 0.7, "rouge": 0.8, "bleu_details": {"info": "data"}},
        ...
        bleu_metric = next(m for m in results[0].metrics if m.metric_name == "bleu")
        assert bleu_metric.details == {"info": "data"}

    def test_skips_raw_score_and_error_keys(self):
        record = Mock(
            metrics={"raw_score": 42, "error": False, "accuracy": 0.9},
            field_name="field1",
            error_message=None,
            passed=True,
        )
    ...
    def test_no_metric_results_empty_list(self):
        record = Mock(
            metrics={"text_only": "hello"},
            field_name="field1",
            error_message=None,
            passed=None,
        )
    ...
    def test_multiple_records(self):
        ...
        results = _build_field_results([record1, record2])
        assert len(results) == 2

    def test_error_message_makes_error_record(self):
        record = Mock(
            metrics={},
            field_name="field1",
