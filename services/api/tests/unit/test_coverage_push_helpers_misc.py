...
class TestReviewEndpointsExtra:
    """Additional review endpoint tests."""

    def test_review_annotation_detail(self, client, test_users, test_db, auth_headers):
        data = _setup_helper_project(test_db, test_users)
        pid = data["project"].id

        # Enable review
        data["project"].review_enabled = True
        test_db.commit()
        ...
        test_db.add(Annotation(
            ...
            task_id=tid,
            project_id=pid,
            result=[{"from_name": "text", "type": "textarea", "value": {"text": ["test"]}}],
        ...
        resp = client.get(
            f"/api/projects/{pid}/review/annotations/{ann_id}",
            headers=auth_headers["admin"],
        )
        assert resp.status_code == 200
