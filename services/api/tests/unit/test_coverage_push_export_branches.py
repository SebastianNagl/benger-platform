...
- Export in label_studio format with annotations that have draft, lead_time, prediction_scores, reviewed_by
    ...
    if add_annotations:
        for i, task in enumerate(tasks):
            ...
            ann = Annotation(
                ...
                reviewed_by=users[0].id if i == 0 else None,
                review_result="approved" if i == 0 else None,
                **extras,
            )
            db.add(ann)
...
class TestExportLabelStudioFormat:
    ...
    def test_export_label_studio_annotation_review(self, client, test_users, test_db, auth_headers):
        data = _setup_project_with_data(test_db, test_users)
        pid = data["project"].id
        ...
        for task_item in content:
            for ann in task_item.get("annotations", []):
                if ann.get("reviewed_by"):
                    found_review = True
                    assert ann["review_result"] == "approved"
        assert found_review
