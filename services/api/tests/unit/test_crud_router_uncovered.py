...
def _mock_project(project_id="proj-1", title="Test", created_by="user-123", is_private=False):
    ...
    p.immediate_evaluation_enabled = False
    p.generation_config = {}
    p.evaluation_config = {}
    p.label_config_version = "v1"
    p.label_config_version_history = []
    p.annotation_time_limit_enabled = False
    p.annotation_time_limit_seconds = None
    p.strict_timer_enabled = False
    p.min_annotations_per_task = 1
    p.questionnaire_enabled = False
    p.questionnaire_config = None
    p.review_enabled = False
    p.review_mode = None
    p.allow_self_review = False
    p.skip_queue = None
    p.llm_model_ids = []
...
class TestUpdateProjectHappyPath:
    ...
    async def test_update_disable_timer(self):
        result = await self._run_update(
            ...
            project_overrides={"strict_timer_enabled": True},
        )
        assert result.id == "proj-1"
