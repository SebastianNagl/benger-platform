...
def _setup_full_project(db, users, *, assignment_mode="open", review_enabled=False,
    ...
    p = Project(
        ...
        annotation_time_limit_enabled=timer_enabled,
        annotation_time_limit_seconds=300 if timer_enabled else None,
...
class TestReviewEndpoints:
    ...
    def test_list_pending_reviews(self, client, test_users, test_db, auth_headers):
        data = _setup_full_project(test_db, test_users, review_enabled=True)
        pid = data["project"].id

        resp = client.get(
            f"/api/projects/{pid}/review/pending",
            headers=auth_headers["admin"],
        )
        assert resp.status_code == 200

    def test_list_pending_reviews_not_enabled(self, client, test_users, test_db, auth_headers):
        data = _setup_full_project(test_db, test_users, review_enabled=False)
        pid = data["project"].id
    ...
    def test_submit_review_approve(self, client, test_users, test_db, auth_headers):
        data = _setup_full_project(test_db, test_users, review_enabled=True)
        pid = data["project"].id
        tid = data["tasks"][0].id
        ...
        resp = client.post(
            f"/api/projects/{pid}/review/annotations/{ann_id}/submit",
            json={"action": "approve"},
            headers=auth_headers["admin"],
        )
        assert resp.status_code == 200

    def test_submit_review_reject(self, client, test_users, test_db, auth_headers):
        data = _setup_full_project(test_db, test_users, review_enabled=True)
        pid = data["project"].id
        tid = data["tasks"][0].id
        ...
        test_db.add(Annotation(
            ...
            task_id=tid,
            project_id=pid,
            result=[{"from_name": "answer", "type": "textarea", "value": {"text": ["bad"]}}],
            completed_by=test_users[1].id,
            was_cancelled=False,
        ))
        test_db.commit()
    ...
    def test_review_stats(self, client, test_users, test_db, auth_headers):
        data = _setup_full_project(test_db, test_users, review_enabled=True)
        pid = data["project"].id

        resp = client.get(
            f"/api/projects/{pid}/review/stats",
            headers=auth_headers["admin"],
        )
        assert resp.status_code == 200
...
class TestTimerEndpoints:
    """Test annotation timer endpoints."""

    def test_start_timer(self, client, test_users, test_db, auth_headers):
        data = _setup_full_project(test_db, test_users, timer_enabled=True)
        pid = data["project"].id
        tid = data["tasks"][0].id

        resp = client.post(
            f"/api/projects/{pid}/tasks/{tid}/start-timer",
            headers=auth_headers["admin"],
        )
        assert resp.status_code == 200

    def test_get_timer_status(self, client, test_users, test_db, auth_headers):
        data = _setup_full_project(test_db, test_users, timer_enabled=True)
        pid = data["project"].id
        tid = data["tasks"][0].id

        resp = client.get(
            f"/api/projects/{pid}/tasks/{tid}/timer-status",
            headers=auth_headers["admin"],
        )
        assert resp.status_code == 200
