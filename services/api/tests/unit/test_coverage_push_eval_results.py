...
- get_immediate_evaluation_status polling branches
- get_sample_result_by_task_model for both model and annotator
- _extract_primary_score priority chain
- _build_field_results with different metric patterns
- _metric_display_name extraction
"""
...
class TestBuildFieldResults:
    """Test _build_field_results helper with different metric patterns."""
    ...
    def test_with_error(self):
        ...
        results = _build_field_results([FakeTE()])
        assert len(results) == 1
    ...
    def test_with_generic_metrics(self):
        ...
        class FakeTE:
            field_name = "answer"
            metrics = {"bleu": 0.7, "rouge": 0.6, "bleu_details": {"ref": "test"}}
            passed = True
            error_message = None
        ...
        assert "bleu_details" not in metric_names  # details excluded
    ...
    def test_empty_metrics(self):
        ...
        results = _build_field_results([FakeTE()])
        assert len(results) == 0  # No metric_results -> no field_results
...
class TestMetricDisplayName:
    ...
    def test_numeric_metric(self):
        ...
        assert _metric_display_name(FakeTE()) == "Bleu"

    def test_skip_raw_score(self):
        from routers.evaluations.results import _metric_display_name

        class FakeTE:
            metrics = {"raw_score": 0.5, "bleu": 0.7}
            field_name = "answer"
    ...
    def test_skip_suffixes(self):
        ...
        class FakeTE:
            metrics = {"bleu_details": {"a": 1}, "bleu_raw": 0.6, "score": 0.7}
            field_name = "answer"

        result = _metric_display_name(FakeTE())
        assert result == "Score"
    ...
    def test_fallback_to_field_name(self):
        ...
        assert _metric_display_name(FakeTE()) == "my_field"

    def test_fallback_none_field_name(self):
        from routers.evaluations.results import _metric_display_name
        ...
        assert _metric_display_name(FakeTE()) == "Evaluation"
...
class TestGetTaskPreview:
    ...
    def test_text_key(self):
        from routers.evaluations.results import _get_task_preview
        assert _get_task_preview({"text": "Sample"}) == "Sample"
    ...
    def test_prompt_key(self):
        from routers.evaluations.results import _get_task_preview
        assert _get_task_preview({"prompt": "Generate"}) == "Generate"

    def test_content_key(self):
        from routers.evaluations.results import _get_task_preview
        assert _get_task_preview({"content": "Body"}) == "Body"

    def test_fallback_string_value(self):
        from routers.evaluations.results import _get_task_preview
        assert _get_task_preview({"custom_key": "Some text"}) == "Some text"
