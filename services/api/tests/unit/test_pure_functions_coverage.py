...
class TestAnswerTypeDetectorExtended:
    ...
    def test_detect_loesung_as_long_text(self):
        from services.evaluation.config import AnswerTypeDetector

        xml = '<View><Loesung name="solution" toName="text"/></View>'
        result = AnswerTypeDetector.detect_from_label_config(xml)
        assert any(r["type"] == "long_text" for r in result)

    def test_detect_gliederung_as_long_text(self):
        from services.evaluation.config import AnswerTypeDetector

        xml = '<View><Gliederung name="outline" toName="text"/></View>'
        result = AnswerTypeDetector.detect_from_label_config(xml)
        assert any(r["type"] == "long_text" for r in result)

    def test_detect_notizen_as_short_text(self):
        from services.evaluation.config import AnswerTypeDetector

        xml = '<View><Notizen name="notes" toName="text"/></View>'
        result = AnswerTypeDetector.detect_from_label_config(xml)
        assert any(r["type"] == "short_text" for r in result)

    def test_detect_angabe_as_span_selection(self):
        from services.evaluation.config import AnswerTypeDetector

        xml = '<View><Angabe name="ref" toName="text"/></View>'
        result = AnswerTypeDetector.detect_from_label_config(xml)
        assert any(r["type"] == "span_selection" for r in result)
...
class TestGetTaskPreview:
    """Tests for routers.evaluations.results._get_task_preview"""
    ...
    def test_input_key(self):
        ...
        assert _get_task_preview({"input": "hello"}) == "hello"

    def test_text_key(self):
        from routers.evaluations.results import _get_task_preview

        assert _get_task_preview({"text": "world"}) == "world"

    def test_question_key(self):
        from routers.evaluations.results import _get_task_preview

        assert _get_task_preview({"question": "what?"}) == "what?"

    def test_prompt_key(self):
        from routers.evaluations.results import _get_task_preview

        assert _get_task_preview({"prompt": "do this"}) == "do this"

    def test_content_key(self):
        from routers.evaluations.results import _get_task_preview

        assert _get_task_preview({"content": "stuff"}) == "stuff"

    def test_priority_input_first(self):
        from routers.evaluations.results import _get_task_preview

        assert _get_task_preview({"text": "b", "input": "a"}) == "a"

    def test_truncation(self):
        from routers.evaluations.results import _get_task_preview

        long = "x" * 200
        result = _get_task_preview({"input": long})
        assert len(result) == 100

    def test_fallback_to_first_string(self):
        from routers.evaluations.results import _get_task_preview

        assert _get_task_preview({"custom_key": "val"}) == "val"

    def test_non_string_values_skipped(self):
        from routers.evaluations.results import _get_task_preview

        assert _get_task_preview({"num": 42, "str_field": "hello"}) == "hello"

    def test_all_non_string_returns_empty(self):
        from routers.evaluations.results import _get_task_preview

        assert _get_task_preview({"a": 1, "b": [1, 2]}) == ""


# ============================================================================
# 8. detect_provider_from_model_id extended
# ============================================================================
...
class TestLabelConfigParserExtended:
    ...
    def test_extract_gliederung(self):
        from services.label_config.parser import LabelConfigParser

        xml = '<View><Gliederung name="outline" toName="text"/></View>'
        fields = LabelConfigParser.extract_fields(xml)
        assert len(fields) == 1
        assert fields[0]["name"] == "outline"
        assert fields[0]["type"] == "Gliederung"

    def test_extract_loesung(self):
        from services.label_config.parser import LabelConfigParser

        xml = '<View><Loesung name="solution" toName="text"/></View>'
        fields = LabelConfigParser.extract_fields(xml)
        assert len(fields) == 1
        assert fields[0]["type"] == "Loesung"
