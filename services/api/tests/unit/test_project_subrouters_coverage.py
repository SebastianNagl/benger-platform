...
class TestListPendingReviews:
    @pytest.mark.asyncio
    async def test_project_not_found(self):
        from routers.projects.reviews import list_pending_reviews
        db = Mock()
        db.query.return_value.filter.return_value.first.return_value = None
        user = Mock()
        request = Mock()
        request.state.organization_context = None
        with pytest.raises(HTTPException) as exc_info:
            await list_pending_reviews(project_id="proj-1", request=request,
                                      page=1, page_size=20,
                                      current_user=user, db=db)
        assert exc_info.value.status_code == 404
...
class TestReviewStats:
    @pytest.mark.asyncio
    async def test_project_not_found(self):
        from routers.projects.reviews import review_stats
        db = Mock()
        db.query.return_value.filter.return_value.first.return_value = None
        user = Mock()
        request = Mock()
        request.state.organization_context = None
        with pytest.raises(HTTPException) as exc_info:
            await review_stats(project_id="proj-1", request=request,
                             current_user=user, db=db)
        assert exc_info.value.status_code == 404


class TestGetAnnotationForReview:
    @pytest.mark.asyncio
    async def test_project_not_found(self):
        from routers.projects.reviews import get_annotation_for_review
        db = Mock()
        db.query.return_value.filter.return_value.first.return_value = None
        user = Mock()
        request = Mock()
        request.state.organization_context = None
        with pytest.raises(HTTPException) as exc_info:
            await get_annotation_for_review(project_id="proj-1",
                                           annotation_id="ann-1",
                                           request=request,
                                           current_user=user, db=db)
        assert exc_info.value.status_code == 404
...
class TestStartTimer:
    @pytest.mark.asyncio
    async def test_project_not_found(self):
        from routers.projects.timer import start_timer
        db = Mock()
        db.query.return_value.filter.return_value.first.return_value = None
        user = Mock()
        request = Mock()
        request.state.organization_context = None
        with pytest.raises(HTTPException) as exc_info:
            await start_timer(project_id="proj-1", task_id="task-1",
                            request=request, current_user=user, db=db)
        assert exc_info.value.status_code == 404


# ============= projects/questionnaire.py =============
