...
class TestUpdateProject:
    ...
    def test_update_disable_time_limit_also_disables_strict(
        self, client, auth_headers, test_db, test_users
    ):
        p = _make_project(
            test_db,
        ...
        assert resp.status_code == 200
        assert resp.json()["strict_timer_enabled"] is False
    ...
    def test_update_review_settings(self, client, auth_headers, test_db, test_users):
        p = _make_project(test_db, test_users[0].id, title="Review Settings")
        resp = client.patch(
            f"/api/projects/{p.id}",
            json={
                "review_enabled": True,
        ...
        assert data["review_mode"] == "independent"
