...
class TestManualModeEnforcement:
    ...
    def test_open_mode_has_no_restriction(
        self, client, auth_headers, assignment_project, test_db
    ):
        """In open mode, annotators can access any task."""
        p = assignment_project
        # Switch to open mode
        p["project"].assignment_mode = "open"
        test_db.commit()

        # Annotator can access any task without assignments
        resp = client.get(
            f"/api/projects/tasks/{p['tasks'][0].id}",
            headers=auth_headers["annotator"],
        )
