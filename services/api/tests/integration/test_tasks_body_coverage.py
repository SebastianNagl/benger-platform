...
- list_project_tasks: pagination, only_labeled/only_unlabeled, only_assigned,
  exclude_my_annotations with auto_submitted, annotator role-based filtering,
...
def _annotate(db, project, tasks, user_id, count=None, cancelled=False, auto_submitted=False):
    anns = []
...
# ===================================================================
# LIST TASKS: exclude_my_annotations with auto_submitted
# ===================================================================

@pytest.mark.integration
class TestListTasksExcludeAnnotations:

    def test_exclude_my_annotations_filters_auto_submitted(self, client, test_db, test_users, auth_headers, test_org):
        p = _project(test_db, test_users[0], test_org)
        tasks = _tasks(test_db, p, test_users[0], count=4)
        # Auto-submitted annotation on task 0
        _annotate(test_db, p, tasks[:1], test_users[0].id, auto_submitted=True)
        test_db.commit()
