...
class TestTaskPreviewHelper:
    def test_get_task_preview_text(self):
        from routers.evaluations.results import _get_task_preview
        assert _get_task_preview({"text": "Hello world"}) == "Hello world"

    def test_get_task_preview_input(self):
        from routers.evaluations.results import _get_task_preview
        assert _get_task_preview({"input": "Input text"}) == "Input text"

    def test_get_task_preview_question(self):
        from routers.evaluations.results import _get_task_preview
        assert _get_task_preview({"question": "What is this?"}) == "What is this?"

    def test_get_task_preview_prompt(self):
        from routers.evaluations.results import _get_task_preview
        assert _get_task_preview({"prompt": "Do something"}) == "Do something"

    def test_get_task_preview_content(self):
        from routers.evaluations.results import _get_task_preview
        assert _get_task_preview({"content": "Content text"}) == "Content text"

    def test_get_task_preview_truncates(self):
        from routers.evaluations.results import _get_task_preview
        long_text = "x" * 200
        result = _get_task_preview({"text": long_text})
        assert len(result) == 100

    def test_get_task_preview_empty(self):
        from routers.evaluations.results import _get_task_preview
        assert _get_task_preview({}) == ""
        assert _get_task_preview(None) == ""

    def test_get_task_preview_fallback_to_first_string(self):
        from routers.evaluations.results import _get_task_preview
        result = _get_task_preview({"custom_field": "Fallback text"})
        assert result == "Fallback text"


# -----------------------------------------------------------------
# Evaluation results endpoint tests
# -----------------------------------------------------------------
