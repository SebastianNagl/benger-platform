...
def _project(db, admin, org, **kw):
    ...
    p = Project(
        ...
        review_enabled=kw.get("review_enabled", False),
        review_mode=kw.get("review_mode", "in_place"),
        allow_self_review=kw.get("allow_self_review", False),
        ...
        immediate_evaluation_enabled=kw.get("immediate_evaluation_enabled", False),
    )
    db.add(p)
    db.flush()
    if org:
...
class TestProjectCRUD:
    ...
    def test_update_disable_time_limit(self, client, test_db, test_users, auth_headers, test_org):
        p = _project(test_db, test_users[0], test_org, annotation_time_limit_enabled=True, strict_timer_enabled=True)
        test_db.commit()
...
class TestReviewWorkflow:
    def test_review_not_enabled(self, client, test_db, test_users, auth_headers, test_org):
        p = _project(test_db, test_users[0], test_org, review_enabled=False)
        test_db.commit()

        resp = client.get(
            f"/api/projects/{p.id}/review/pending",
            headers={**auth_headers["admin"], "X-Organization-Context": test_org.id},
        )
        assert resp.status_code == 400

    def test_list_pending_reviews(self, client, test_db, test_users, auth_headers, test_org):
        p = _project(test_db, test_users[0], test_org, review_enabled=True)
        t = _task(test_db, p, test_users[0])
    ...
    def test_review_stats(self, client, test_db, test_users, auth_headers, test_org):
        p = _project(test_db, test_users[0], test_org, review_enabled=True)
        t = _task(test_db, p, test_users[0])
        _ann(test_db, t, p, test_users[0])
        test_db.commit()
        ...
        resp = client.get(
            f"/api/projects/{p.id}/review/stats",
            headers={**auth_headers["admin"], "X-Organization-Context": test_org.id},
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_get_annotation_for_review(self, client, test_db, test_users, auth_headers, test_org):
        p = _project(test_db, test_users[0], test_org, review_enabled=True)
        t = _task(test_db, p, test_users[0])
        ann = _ann(test_db, t, p, test_users[1])
        test_db.commit()

        resp = client.get(
            f"/api/projects/{p.id}/review/annotations/{ann.id}",
            headers={**auth_headers["admin"], "X-Organization-Context": test_org.id},
        )
        assert resp.status_code == 200

    def test_submit_review_approve(self, client, test_db, test_users, auth_headers, test_org):
        p = _project(test_db, test_users[0], test_org, review_enabled=True)
        t = _task(test_db, p, test_users[0])
        ann = _ann(test_db, t, p, test_users[1])
        test_db.commit()
        ...
        resp = client.post(
            f"/api/projects/{p.id}/review/annotations/{ann.id}/submit",
            json={"action": "approve", "comment": "Good job"},
            headers={**auth_headers["admin"], "X-Organization-Context": test_org.id},
        )
        assert resp.status_code == 200
        data = resp.json()
        assert data["review_result"] == "approved"

    def test_submit_review_reject(self, client, test_db, test_users, auth_headers, test_org):
        p = _project(test_db, test_users[0], test_org, review_enabled=True)
        t = _task(test_db, p, test_users[0])
        ann = _ann(test_db, t, p, test_users[1])
        test_db.commit()
    ...
    def test_submit_review_fix(self, client, test_db, test_users, auth_headers, test_org):
        p = _project(test_db, test_users[0], test_org, review_enabled=True)
        t = _task(test_db, p, test_users[0])
        ann = _ann(test_db, t, p, test_users[1])
        test_db.commit()
        ...
        resp = client.post(
            ...
            json={
                "action": "fix",
                "result": [{"from_name": "answer", "to_name": "text", "type": "choices", "value": {"choices": ["Nein"]}}],
                "comment": "Fixed",
            },
            headers={**auth_headers["admin"], "X-Organization-Context": test_org.id},
        )
    ...
    def test_review_diff(self, client, test_db, test_users, auth_headers, test_org):
        p = _project(test_db, test_users[0], test_org, review_enabled=True)
        t = _task(test_db, p, test_users[0])
        ann = _ann(test_db, t, p, test_users[1])
        ann.reviewed_by = test_users[0].id
        ann.review_result = "approved"
        ann.reviewed_at = datetime.now(timezone.utc)
        test_db.commit()

        resp = client.get(
            f"/api/projects/{p.id}/review/annotations/{ann.id}/diff",
            headers={**auth_headers["admin"], "X-Organization-Context": test_org.id},
        )
        assert resp.status_code == 200

    def test_self_review_blocked(self, client, test_db, test_users, auth_headers, test_org):
        p = _project(test_db, test_users[0], test_org, review_enabled=True, allow_self_review=False)
        t = _task(test_db, p, test_users[0])
