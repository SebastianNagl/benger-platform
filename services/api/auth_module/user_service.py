...
def delete_user(db: Session, user_id: str) -> bool:
    ...
    fallback_user = (
        db.query(User).filter(User.username == "pschOrr95", User.is_superadmin == True).first()  # noqa: E712
    )
    if not fallback_user:
        fallback_user = (
            db.query(User).filter(User.is_superadmin == True, User.id != user_id).first()  # noqa: E712
        )
