...
async function createFullFeaturedProject(
      ...
      const configResp = await fetch(`/api/projects/${projectId}`, {
        ...
        body: JSON.stringify({
          ...
          conditional_instructions: [
            {
              id: 'variant_a',
              content: 'VARIANT A: Focus on legal implications.',
              weight: 50,
            },
            {
              id: 'variant_b',
              content: 'VARIANT B: Focus on factual accuracy.',
              weight: 50,
            },
          ],
  ...
  const importResult = await page.evaluate(importTasksInBrowser, {
    ...
    tasks: Array.from({ length: taskCount }, (_, i) => ({
      data: {
        text: `Legal analysis task ${i + 1}: Evaluate the contractual obligations in this scenario.`,
      },
    })),
  })
  if (!importResult.success) throw new Error(`Import: ${importResult.error}`)
  return projectId
}
...
test.describe('Randomized Order with Annotation Features', () => {
  ...
  test('timer displays and counts down with randomized task order @extended', async () => {
    ...
    const dismissButton = page
      .locator('button')
      .filter({ hasText: /Annotation starten/ })
  ...
  test('conditional instruction variant is shown and deterministic with randomized order', async () => {
    ...
    if (hasStart) {
      ...
      const variantVisibleOnStart = await variantOnStart
        .first()
    ...
    const hasVariantA = await variantA
      .isVisible({ timeout: 5000 })
      .catch(() => false)
    const hasVariantB = await variantB
      .isVisible({ timeout: 5000 })
      .catch(() => false)
    ...
    console.log(
      `Variant A visible: ${hasVariantA}, Variant B visible: ${hasVariantB}`,
    )
    ...
    if (variantShown) {
      ...
      await page.waitForTimeout(3000)
      const variantAAfterReload = await page
        .locator('text=VARIANT A')
        .isVisible({ timeout: 5000 })
        .catch(() => false)
      const variantBAfterReload = await page
        .locator('text=VARIANT B')
        .isVisible({ timeout: 5000 })
        .catch(() => false)
      ...
      if (variantAAfterReload || variantBAfterReload) {
        console.log(
          `After reload - Variant A: ${variantAAfterReload}, Variant B: ${variantBAfterReload}`,
        )
        expect(sameVariant).toBe(true)
        console.log(
          'Same variant shown after reload (deterministic hash confirmed)',
        )
      } else {
        console.log(
          'Variant not visible after reload (may require clicking Start again)',
        )
      }
    } else {
      // Variant might be shown in a different UI flow (e.g., dismissed modal)
      console.log(
        'No variant text visible in current view - checking general instructions',
      )
      const generalInstructions = page.locator('text=General instructions')
      const hasGeneral = await generalInstructions
        .isVisible({ timeout: 3000 })
        .catch(() => false)
  ...
  test('post-annotation questionnaire appears after submission with randomized order', async () => {
    ...
    const choiceOption = page
      ...
      .or(
        page.locator('label').filter({ hasText: /positive|negative|neutral/i }),
      )
      .first()
    ...
    const hasQuestionnaire = await questionnaireTitle
      .first()
      .isVisible({ timeout: 10000 })
      .catch(() => false)
    ...
    if (hasQuestionnaire) {
      ...
      const canSelect = await confidentOption
        .isVisible({ timeout: 5000 })
        .catch(() => false)
      if (canSelect) {
        ...
        const submitQuestionnaire = page
          ...
          .first()
        const canSubmit = await submitQuestionnaire
          .isVisible({ timeout: 5000 })
          .catch(() => false)
        if (canSubmit) {
  ...
  test('immediate evaluation modal appears after submission with randomized order', async () => {
    ...
    const hasEvalModal = await evalModal
      .first()
    ...
    if (hasEvalModal) {
      ...
      const evalContent = page.locator(
        'text=/Evaluating|Auswerten|Results|Ergebnis|saved|gespeichert|Error|Fehler/i',
      )
      const hasContent = await evalContent
        .first()
        .isVisible({ timeout: 10000 })
        .catch(() => false)
    ...
    const counterVisible = await newCounter
      .isVisible({ timeout: 10000 })
      .catch(() => false)
