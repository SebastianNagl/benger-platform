...
test.describe('Org Role Permissions', () => {
  test('contributor can edit project settings but not delete', async ({
    browser,
  }) => {
    ...
    const projectId = await adminPage.evaluate(
      ...
      {
        name: 'Role Permissions E2E Test',
    ...
    console.log(
      `Contributor PATCH project: ${updateResult.status} (expected 200)`,
    )
    ...
    console.log(
      `Contributor DELETE project: ${deleteResult.status} (expected 403)`,
    )
  ...
  test('annotator cannot access admin features', async ({ browser }) => {
    ...
    const projectId = await adminPage.evaluate(
      ...
      {
        name: 'Annotator Deny E2E Test',
        description: 'Testing annotator restrictions',
        orgId,
      },
    )
    ...
    console.log(
      `Annotator PATCH project: ${updateResult.status} (expected 403)`,
    )
    ...
    console.log(
      `Annotator assign tasks: ${assignResult.status} (expected 400 or 403)`,
    )
    ...
    expect(deleteResult.status).toBe(403)
    console.log(
      `Annotator DELETE project: ${deleteResult.status} (expected 403)`,
    )
