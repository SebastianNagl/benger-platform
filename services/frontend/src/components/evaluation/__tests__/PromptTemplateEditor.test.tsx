...
describe('PromptTemplateEditor', () => {
  it('renders textarea with current value', () => {
    render(
      <Harness initial="Hello {{fall}}" knownVariables={['fall', 'answer']} />,
    )
  ...
  it('shows variable pills with ✓ for variables present in the template', () => {
    render(
      <Harness initial="Fall: {{fall}}" knownVariables={['fall', 'answer']} />,
    )
    // fall is used → ✓; answer is not → no ✓
    expect(
      screen.getByRole('button', { name: /\{\{fall\}\}/ }).textContent,
    ).toMatch(/✓/)
    expect(
      screen.getByRole('button', { name: /\{\{answer\}\}/ }).textContent,
    ).not.toMatch(/✓/)
  })

  it('warns when template references variables outside knownVariables', () => {
    render(<Harness initial="Use {{unmapped_var}}" knownVariables={['fall']} />)
    expect(
    ...
    const codeMatches = screen
      .getAllByText('unmapped_var')
      .filter((el) => el.tagName.toLowerCase() === 'code')
  ...
  it('inserts variable token at cursor when pill clicked', async () => {
    ...
    render(<Harness initial="" knownVariables={['fall']} />)
