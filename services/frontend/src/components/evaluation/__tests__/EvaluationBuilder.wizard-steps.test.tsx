  ...
  useModels: () => ({
    models: [
      ...
      },
      {
        ...
        provider: 'anthropic',
        default_config: {
          reasoning_config: { parameter: 'thinking_budget', default: 16000 },
        },
      },
    ],
    loading: false,
...
jest.mock('@/lib/api', () => ({
  api: {
    get: jest.fn().mockResolvedValue({
      data: {
        field_types: {
          model_answer: { type: 'short_text', name: 'model_answer' },
        },
      },
    }),
  ...
  FieldMappingEditor: ({ value, onChange }: any) => (
    <div data-testid="field-mapping-editor">
      <button
        ...
        onClick={() => onChange({ ...value, key1: 'val1' })}
      >
        Add Mapping
      </button>
    </div>
...
describe('EvaluationBuilder wizard step rendering', () => {
  ...
  describe('Step 1 - Metric selection', () => {
    it('renders metric groups with selectable buttons', async () => {
      ...
      expect(
        screen.getByText('evaluationBuilder.steps.metric.title'),
      ).toBeInTheDocument()
      expect(
        screen.getByText('evaluationBuilder.steps.metric.description'),
      ).toBeInTheDocument()
  ...
  describe('Step 2 - Prediction fields', () => {
    it('renders bulk selectors and individual fields', async () => {
      ...
      expect(
        screen.getByText('evaluationBuilder.fields.bulkSelection'),
      ).toBeInTheDocument()
      expect(
        screen.getByText('evaluationBuilder.fields.modelResponseFields'),
      ).toBeInTheDocument()
      expect(
        screen.getByText('evaluationBuilder.fields.humanAnnotationFields'),
      ).toBeInTheDocument()
    })
    ...
    it('shows empty fields message when no prediction fields exist', async () => {
      ...
      expect(
        screen.getByText('evaluationBuilder.fields.noFieldsDetected'),
      ).toBeInTheDocument()
    })
  })
  ...
  describe('Step 3 - Reference fields', () => {
    it('renders reference field checkboxes', async () => {
      ...
      expect(
        screen.getByText('evaluationBuilder.steps.referenceFields.title'),
      ).toBeInTheDocument()
    })
    ...
    it('shows no-reference-fields message when none available', async () => {
      ...
      expect(
        screen.getByText('evaluationBuilder.fields.noReferenceFields'),
      ).toBeInTheDocument()
    })
    ...
    it('shows multiple references info when 2+ selected', async () => {
      ...
      expect(
        screen.getByText('evaluationBuilder.fields.multipleReferencesTitle'),
      ).toBeInTheDocument()
    })
  })
  ...
  describe('Step 4 - Parameters: LLM Judge Classic', () => {
    ...
    it('changes judge model and updates temperature defaults', async () => {
      ...
      const selects = screen.getAllByRole('combobox')
      const judgeSelect = selects.find((s) =>
        s.querySelector('option[value="o3"]'),
      )
      if (judgeSelect) {
        ...
        if (sel.querySelector('option[value="claude-opus-4-5-20251101"]')) {
          await act(async () => {
            fireEvent.change(sel, {
              target: { value: 'claude-opus-4-5-20251101' },
            })
          })
          break
        }
    ...
    it('renders reasoning effort for o-series models', async () => {
      ...
      const allSelects = screen.getAllByRole('combobox')
      const effortSelect = allSelects.find((s) =>
        s.querySelector('option[value="high"]'),
      )
      if (effortSelect) {
    ...
    it('changes max tokens', async () => {
      ...
      const spinbuttons = screen.getAllByRole('spinbutton')
      const maxTokensInput = spinbuttons.find(
        (s) => (s as HTMLInputElement).value === '500',
      )
      if (maxTokensInput) {
  ...
  describe('Step 4 - Parameters: LLM Judge Custom', () => {
    it('renders custom LLM judge with full configuration', async () => {
      ...
      expect(
        screen.getByText(/Configure your own evaluation prompt/),
      ).toBeInTheDocument()
    ...
    it('changes score scale', async () => {
      ...
      const scaleSelect = selects.find((s) =>
        s.querySelector('option[value="0-1"]'),
      )
      if (scaleSelect) {
  ...
  describe('Step 5 - Review', () => {
    it('renders review for exact_match with basic fields', async () => {
      ...
      expect(
        screen.getByText(/evaluationBuilder\.review\.metric/),
      ).toBeInTheDocument()
      expect(
        screen.getByText(/evaluationBuilder\.review\.predictionFields/),
      ).toBeInTheDocument()
      expect(
        screen.getByText(/evaluationBuilder\.review\.referenceFields/),
      ).toBeInTheDocument()
    })
    ...
    it('renders review for bleu and shows parameters JSON', async () => {
      ...
      expect(
        screen.getByText(/evaluationBuilder\.review\.parameters/),
      ).toBeInTheDocument()
    })
  })
  ...
  describe('Custom evaluation name', () => {
    ...
    it('edit prefills EMPTY for an auto-generated model-enriched name (so it recomputes, not stale)', async () => {
      ...
      const input = await openEditToReview(
        user,
        'Classic LLM Judge (gpt-4o ×3)',
      )
  ...
  describe('Run evaluation modal', () => {
    ...
    it('does not render an inner run button (removed in favor of page-level trigger)', () => {
      render(
        <EvaluationBuilder
          {...defaultProps}
          evaluations={evaluationsFixture}
        />,
      )
      const runBtn = screen
  ...
  describe('Empty state', () => {
    it('shows empty state when no evaluations configured', () => {
      ...
      expect(
        screen.getByText('evaluationBuilder.emptyState.title'),
      ).toBeInTheDocument()
      expect(
        screen.getByText('evaluationBuilder.emptyState.description'),
      ).toBeInTheDocument()
    })
    ...
    it('hides empty state when wizard is open', async () => {
      ...
      await user.click(screen.getByTestId('add-evaluation-button'))
      expect(
        screen.queryByText('evaluationBuilder.emptyState.title'),
  ...
  describe('Cancel wizard', () => {
    it('closes wizard when cancel button is clicked', async () => {
      ...
      const cancelBtn = screen
        .getAllByRole('button')
        .find((b) => b.textContent === 'evaluationBuilder.cancel')
      if (cancelBtn) {
  ...
  describe('Field toggle with LLM Judge auto-detect', () => {
    it('auto-detects answer type when selecting prediction field for llm_judge', async () => {
      ...
      const judgeBtn =
        screen.queryByTestId('metric-button-llm_judge') ||
        screen.queryByTestId('metric-button-llm_judge_classic')
      if (judgeBtn) {
  ...
  describe('Saving state', () => {
    ...
    it('does not render the inner "running" label', () => {
      ...
      expect(
        screen.queryByText('evaluationBuilder.runEvaluation.running'),
      ).not.toBeInTheDocument()
    })
  })
  ...
  describe('Evaluation configs count', () => {
    it('does not render the inner "configs will be evaluated" counter', () => {
      ...
      expect(
        screen.queryByText(
          /evaluationBuilder.runEvaluation.configsWillBeEvaluated/,
        ),
      ).not.toBeInTheDocument()
    })
  })
