      ...
      <ul className="space-y-0.5">
        {lines.map((line) => (
          ...
          >
            {line.swatchClass && (
              <span
                className={`inline-block h-2.5 w-2.5 rounded-sm ${line.swatchClass}`}
                aria-hidden="true"
              />
            )}
            <span>{line.label}</span>
            <span className="ml-auto pl-3 text-zinc-900 tabular-nums dark:text-white">
              {line.value}
            </span>
          </li>
        ))}
...
export function ChartLegend({
  ...
  items: Array<{ label: string; swatchClass: string }>
}) {
  return (
    <ul
      className="mt-2 flex flex-wrap gap-4 text-xs text-zinc-600 dark:text-zinc-300"
      data-testid="chart-legend"
    >
      {items.map((item) => (
        <li key={item.label} className="flex items-center gap-1.5">
          <span
            className={`inline-block h-2.5 w-2.5 rounded-sm ${item.swatchClass}`}
            aria-hidden="true"
          />
          {item.label}
        </li>
