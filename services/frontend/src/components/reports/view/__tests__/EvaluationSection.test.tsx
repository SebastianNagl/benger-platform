...
const registry = {
  llm_judge_falloesung: {
    ...
    display_scale: '0-100' as const,
  },
}

function renderSection(
  chartsConfig?: ReportChartsConfig | null,
  extra: Partial<React.ComponentProps<typeof EvaluationSection>> = {},
) {
  return render(
    <EvaluationSection
      snapshot={FIX}
...
describe('EvaluationSection', () => {
  it('renders methods, both tables, charts, interpretation and conclusions', () => {
    ...
    expect(
      screen.getByText('Verteilung der Notenpunkte: Menschen vs. Modelle'),
    ).toBeInTheDocument()
  ...
  it('switches configs via the selector', () => {
    ...
    expect(
      within(screen.getByTestId('ranking-table-models')).getByTestId(
        'ranking-row',
      ),
    ).toHaveTextContent('79,1 / 100')
  ...
  it('applies hidden_subjects, show_humans and show_distribution', () => {
    ...
    expect(screen.queryByTestId('distribution-chart')).not.toBeInTheDocument()
    expect(
      screen.queryByTestId('per-subject-distribution'),
  ...
  it('falls back to the primary metric distribution when there is no grade metric', () => {
    ...
    expect(
      screen.getByText(
        'Verteilung (Falllösung LLM Judge): Menschen vs. Modelle',
      ),
    ).toBeInTheDocument()
  ...
  it('renders a quiet note when no primary metric resolves', () => {
    render(
      <EvaluationSection
        snapshot={{ ...FIX, primary_metric: null }}
        registry={{}}
        locale="de"
        t={t}
      />,
