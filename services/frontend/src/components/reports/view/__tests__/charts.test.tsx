...
describe('DistributionChart', () => {
  it('renders two series and toggles between share and count', () => {
    ...
    expect(
      screen.queryByText(/Keine menschlichen Abgaben/),
  ...
  it('notes a missing human series and honours defaultMode', () => {
    const noHumans = {
      ...gradeDist,
      by_kind: {
        model: gradeDist.by_kind.model,
        human: gradeDist.by_kind.human.map(() => 0),
      },
    }
    render(
      <DistributionChart
        distribution={noHumans}
        valueLabel="NP"
        title="V"
        locale="de"
        t={t}
    ...
    expect(screen.getByTestId('distribution-chart')).toHaveAttribute(
      'data-mode',
      'count',
    )
...
describe('PerSubjectDistribution', () => {
  ...
  it('tooltip shows share and count', () => {
    ...
    rerender(
      <SubjectBinTooltip active={false} locale="de" t={t} valueLabel="NP" />,
    )
    expect(screen.queryByTestId('chart-tooltip')).not.toBeInTheDocument()
  })
})
...
describe('MeanBarChart', () => {
  it('renders a horizontal bar chart in ranking order with error bars', () => {
    render(
      <MeanBarChart
        rows={modelRows}
        scale="0-100"
        metricLabel="Falllösung LLM Judge"
        locale="de"
        t={t}
      />,
    )
    ...
    expect(
      screen.getByText(/Falllösung LLM Judge je Modell/),
    ).toBeInTheDocument()
    ...
    expect(screen.getByTestId('error-bar')).toHaveAttribute(
      'data-datakey',
      'std',
    )
    expect(screen.getByTestId('error-bar')).toHaveAttribute(
  ...
  it('maps rows to chart data and renders nothing when empty', () => {
    ...
    const { container } = render(
      <MeanBarChart rows={[]} scale="raw" metricLabel="x" locale="de" t={t} />,
    )
    expect(container).toBeEmptyDOMElement()
  })
