 ...
 * cases for TextArea/Choices/Choice/Angabe missing attributes,
...
describe('validateParsedConfig', () => {
  ...
  it('should validate Angabe requires name, toName, and value', () => {
    const config: ParsedComponent = {
      type: 'View',
      props: {},
      children: [{ type: 'Angabe', props: {}, children: [] }],
    }
    const result = validateParsedConfig(config)
    expect(result.errors.some((e) => e.includes('Angabe component requires \'name\''))).toBe(true)
    expect(result.errors.some((e) => e.includes('Angabe component requires \'toName\''))).toBe(true)
    expect(result.errors.some((e) => e.includes('Angabe component requires \'value\''))).toBe(true)
  })
