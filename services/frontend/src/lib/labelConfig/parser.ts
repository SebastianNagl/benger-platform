...
function validateComponent(
  ...
  switch (component.type) {
    ...
    case 'Angabe':
      if (!component.props.name) {
        errors.push(
          `${currentPath}: Angabe component requires 'name' attribute`
        )
      }
      if (!component.props.toName) {
        errors.push(
          `${currentPath}: Angabe component requires 'toName' attribute`
        )
      }
      if (!component.props.value) {
        errors.push(
          `${currentPath}: Angabe component requires 'value' attribute for data binding`
        )
      }
      break
  }
