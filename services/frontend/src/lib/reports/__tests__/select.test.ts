...
describe('select helpers', () => {
  ...
  describe('selectVisibleConfigs', () => {
    ...
    it('filters by visible_configs in snapshot order', () => {
      const out = selectVisibleConfigs(FIX, {
        visible_configs: ['cfg-bleu', 'cfg-judge-mini'],
      })
  ...
  describe('selectPrimary', () => {
    ...
    it('honours editor overrides', () => {
      expect(
        selectPrimary(FIX, { primary_config_id: 'cfg-judge-mini' }).configId,
      ).toBe('cfg-judge-mini')
    ...
    it('falls back to the first visible config for the metric when the preferred one is hidden', () => {
      const sel = selectPrimary(FIX, {
        visible_configs: ['cfg-judge-mini', 'cfg-bleu'],
      })
    ...
    it('ignores a primary_config_id that scores a different metric', () => {
      expect(
        selectPrimary(FIX, { primary_config_id: 'cfg-bleu' }).configId,
      ).toBe('cfg-judge-sonnet')
    })

    it('returns nulls when no metric can be resolved', () => {
      expect(
        selectPrimary({ ...FIX, primary_metric: null }, undefined),
      ).toEqual({
        metric: null,
        configId: null,
        gradeMetric: null,
      ...
      expect(
        selectPrimary(FIX, { visible_configs: ['cfg-bleu'] }).configId,
      ).toBeNull()
    })
  })
  ...
  it('lists selector options for the primary metric', () => {
    expect(
      configsForMetric(FIX.configs, 'llm_judge_falloesung').map((c) => c.id),
    ).toEqual(['cfg-judge-sonnet', 'cfg-judge-mini'])
  ...
  describe('rankSeries', () => {
    ...
    it('breaks ties by n, then label', () => {
      const series = [
        {
          subject: { id: 'b', kind: 'model' as const, label: 'B' },
          config_id: 'c',
          metrics: { m: { mean: 1, n: 5 } },
        },
        {
          subject: { id: 'a', kind: 'model' as const, label: 'A' },
          config_id: 'c',
          metrics: { m: { mean: 1, n: 5 } },
        },
        {
          subject: { id: 'z', kind: 'model' as const, label: 'Z' },
          config_id: 'c',
          metrics: { m: { mean: 1, n: 9 } },
        },
      ]
      expect(
        rankSeries(series, 'c', 'm', 'model').map((r) => r.subject.id),
      ).toEqual(['z', 'a', 'b'])
    })

    it('separates humans, excludes hidden subjects and rows without the metric', () => {
      const humans = rankSeries(
        FIX.series,
        'cfg-judge-sonnet',
        'llm_judge_falloesung',
        'human',
      )
      expect(humans.map((r) => r.subject.label)).toEqual([
        'KindAlly',
        'BraveOtter',
      ])

      const hidden = rankSeries(
        FIX.series,
        'cfg-judge-sonnet',
        'llm_judge_falloesung',
        'model',
      ...
      expect(hidden.map((r) => r.subject.label)).toEqual([
        'Claude Opus 4.7',
        'DeepSeek V4 Flash',
        'Llama 4 Maverick',
      ])
      ...
      expect(
        rankSeries(FIX.series, 'cfg-judge-sonnet', 'bleu', 'model'),
      ).toEqual([])
  ...
  describe('otherMetricColumns', () => {
    ...
    it('drops primary, grade and hidden companions', () => {
      ...
      expect(otherMetricColumns(rows, 'llm_judge_falloesung', null)).toEqual([
        'llm_judge_falloesung_grade_points',
      ])
    })
  ...
  describe('distributions', () => {
    ...
    it('yields zero shares for an empty kind', () => {
      const d = {
        ...FIX.distributions[0],
        by_kind: { model: FIX.distributions[0].by_kind.model, human: [] },
      }
      const series = toPercentSeries(d)
    ...
    it('builds per-subject histograms for top ranked rows with data', () => {
      ...
      expect(subjects[0].bins[13]).toEqual({
        bin: 13,
        label: '13',
        count: 5,
        pct: 33.3,
