...
export const TROUBLESHOOTING_GUIDES: HowToGuide[] = [
  ...
  },
  {
    ...
    steps: {
      de: [
        '**ILIAS: „ILIAS kennt die Person nicht“ oder „User not available“**: Der ILIAS-Provider identifiziert die Personen per *E-Mail-Adresse*. In ILIAS 10.9 scheitert in diesem Modus jede Notenübertragung. Die Karte der Anbindung warnt dann mit **Notenübertragung nach ILIAS scheitert**. Die ILIAS-Administration stellt beim Provider *Identifikation der Person* auf **ID des ILIAS-Kontos …** oder **Hash@ILIAS-Plattform-ID.ilias** um. Danach ändert sich die Kennung jeder Person. Beim nächsten Start stimmt jede Person erneut zu und erhält ein neues Konto. Bisherige Abgaben bleiben beim alten Konto.',
        ...
        '**Anbindung ausgeschaltet**: Während der Schalter **Anbindung aus** zeigt, gehen keine Noten hinaus. Schalten Sie die Anbindung ein und klicken Sie auf **Erneut senden**.',
      ],
      en: [
        '**ILIAS: “ILIAS kennt die Person nicht” or “User not available”**: the ILIAS provider identifies people by *E-Mail Address*. In ILIAS 10.9 every grade transfer fails in this mode. The connection card then warns with **Grade transfer to ILIAS fails**. The ILIAS administration switches the provider’s *User identification* to **ILIAS user id …** or **Hash@ILIAS-Plattform-ID.ilias**. After that, the identifier of every person changes. On the next launch each person consents again and gets a new account. Earlier submissions stay with the old account.',
        ...
        '**Connection switched off**: while the switch reads **Connection off**, no grades go out. Switch the connection on and click **Send again**.',
      ],
    },
    tips: {
      de: [
