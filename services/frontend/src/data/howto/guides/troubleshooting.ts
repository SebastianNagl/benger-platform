  ...
  },
  {
    ...
    steps: {
      de: [
        ...
        '`registration_disabled`: Die Organisation hat die Anbindung abgeschaltet. Eine abgeschaltete Anbindung zeigt bei jedem Start diesen Code. Die Admins Ihrer Organisation schalten sie mit **Anbindung aktivieren** wieder ein.',
        ...
        '`deployment_disabled`: Die Deployment-ID ist in der Anbindung eingetragen, aber abgeschaltet. Die Admins Ihrer Organisation schalten sie unter **Deployments** mit **Deployment einschalten** wieder ein.',
        ...
        '`user_inactive`: Das Konto der Person ist in dieser Plattform deaktiviert. Nur der Betreiber der Plattform kann es wieder freischalten. Lehrende wenden sich an ihn.',
        '`membership_removed`: Ein Admin hat die Person aus der Organisation der Anbindung entfernt. Ein Start aus der Lernplattform stellt die Mitgliedschaft nicht wieder her. Soll die Person wieder Zugang haben, laden die Admins Ihrer Organisation sie mit **Mitglied einladen** an die E-Mail-Adresse ihres Kontos ein. Nimmt die Person die Einladung angemeldet an, ist die Mitgliedschaft wieder aktiv. Hat das Konto noch kein Passwort, legt die Person vorher mit **Passwort vergessen?** eines fest.',
        ...
        '`account_not_linkable`: Dieses Konto darf nicht mit einer Lernplattform verknüpft werden, zum Beispiel ein Konto mit Administrationsrechten für die ganze Plattform. Öffnen Sie die Aktivität erneut und wählen Sie **Mit separatem Konto fortfahren**. Besteht die Verknüpfung schon, lösen die Admins Ihrer Organisation sie unter **LMS-Konten** mit **Verknüpfung lösen**.',
      ...
      en: [
        ...
        '`registration_disabled`: the organization has switched off the connection. A switched-off connection shows this code on every launch. The admins of your organization switch it back on with **Enable connection**.',
        ...
        '`deployment_disabled`: the deployment ID is registered in the connection but switched off. The admins of your organization switch it back on under **Deployments** with **Turn deployment on**.',
        ...
        '`account_not_linkable`: this account may not be linked to a learning platform, for example an account with administration rights for the whole platform. Open the activity again and choose **Continue with a separate account**. If the link already exists, the admins of your organization remove it under **LMS accounts** with **Unlink**.',
