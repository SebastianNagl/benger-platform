...
export const INTEGRATION_GUIDES: HowToGuide[] = [
  {
    ...
    steps: {
      de: [
        ...
        '**Lernplattform registriert das Tool**: In Moodle fügt die Administration den Link unter *Website-Administration → Plugins → Externes Tool → Tools verwalten* bei **LTI Advantage hinzufügen** ein und aktiviert das Tool. ILIAS ab Version 10.9 nimmt den Link beim Anlegen eines LTI-Konsumenten an. Danach aktiviert die ILIAS-Administration beim Provider **Advanced Grading Services**, sonst kommen keine Noten an. Die Anbindung steht im Panel und ist **sofort aktiv**.',
      ...
      en: [
        ...
        '**The learning platform registers the tool**: in Moodle the administration pastes the link under *Site administration → Plugins → External tool → Manage tools* into **Add LTI Advantage** and activates the tool. ILIAS 10.9 and later accepts the link when an LTI consumer is created. The ILIAS administration then switches on **Advanced Grading Services** on the provider, or no grades arrive. The connection shows up in the panel and is **active at once**.',
