...
export const INTEGRATION_GUIDES: HowToGuide[] = [
  {
    ...
    steps: {
      ...
      en: [
        ...
        '**The learning platform registers the tool**: in Moodle the administration pastes the link under *Site administration → Plugins → Activity modules → External tool → Manage tools* into **Add LTI Advantage**. If Moodle asks whether to update an existing tool, they choose **Register as a new external tool**, unless they replace an earlier connection on purpose. Then they activate the tool. ILIAS 10.9 and later accepts the link when an LTI consumer is created. The ILIAS administration then switches on **Advanced Grading Services** (German ILIAS: *Erweiterte Benotungsdienste*) on the provider, or no grades arrive. They also set the identification and the name (see the tips). The connection shows up in the panel and is **active at once**.',
    ...
    tips: {
      de: [
        ...
        'ILIAS: Die Administration legt einen globalen Provider an und wählt bei *Verfügbarkeit* **in neuen und bestehenden Objekten**. Unter *Identifikation der Person* wählt sie einen Modus ohne E-Mail-Adresse, am besten **ID des ILIAS-Kontos …**. Mehr Pseudonymität bietet **Hash@ILIAS-Plattform-ID.ilias**. Beim *Anmeldename* wählt sie **Vollständiger Name**. Das gilt auch nach einer Registrierung per Link. Konten aus ILIAS tragen dann den vollen Namen, aber keine E-Mail-Adresse. Es gibt also keine Aktivierungsmail und keine Verknüpfung mit bestehenden Konten.',
      ...
      en: [
        ...
        'ILIAS: the administration creates a global provider and sets *Availability* to **For Creating Objects** (German ILIAS: *in neuen und bestehenden Objekten*). Under *User identification* they choose a mode without the email address, best **ILIAS user id …** (German ILIAS: *ID des ILIAS-Kontos …*). **Hash@ILIAS-Plattform-ID.ilias** gives more pseudonymity. For the *User name* they choose **Entire name** (German ILIAS: *Vollständiger Name*). This also applies after a registration by link. Accounts from ILIAS then carry the full name but no email address. So there is no activation mail and no linking to existing accounts.',
  ...
  },
  {
    ...
    tips: {
      de: [
        ...
        'Neue Konten mit zustellbarer Adresse erhalten nach der Zustimmung eine E-Mail zum Festlegen eines Passworts. Konten aus ILIAS haben keine Adresse und erhalten diese E-Mail nicht. Wer sich ohne Lernplattform anmelden möchte, hinterlegt später in der App eine Adresse.',
      ...
      en: [
        ...
        'New accounts with a deliverable address get an email to set a password after consent. Accounts from ILIAS have no address and do not get this email. Anyone who wants to sign in without the learning platform adds an address in the app later.',
    ...
    pitfalls: {
      de: [
        'Stellen Sie in ILIAS vor dem Start die **Identifikation der Person** auf einen Modus ohne E-Mail-Adresse, am besten **ID des ILIAS-Kontos …**. Mehr Pseudonymität bietet **Hash@ILIAS-Plattform-ID.ilias**. Beim *Anmeldename* wählen Sie **Vollständiger Name**. Der Modus *E-Mail-Adresse* ist ungeeignet, weil in ILIAS 10.9 damit jede Notenübertragung scheitert. Konten aus ILIAS tragen deshalb den Namen, aber keine E-Mail-Adresse: keine Aktivierungsmail, keine Verknüpfung mit bestehenden Konten. Ändern Sie die Identifikation danach nicht mehr. Ein Wechsel ändert die Kennung jeder Person und trennt alle Verknüpfungen.',
      ...
      en: [
        'Before go-live, set the **User identification** in ILIAS to a mode without the email address, best **ILIAS user id …**. **Hash@ILIAS-Plattform-ID.ilias** gives more pseudonymity. For the *User name*, choose **Entire name**. The mode *E-Mail Address* is not suitable, because in ILIAS 10.9 every grade transfer fails with it. Accounts from ILIAS therefore carry the name but no email address: no activation mail, no linking to existing accounts. Do not change the identification afterwards. A change alters every person’s identifier and detaches every link.',
