...
describe('ProjectDetailPage', () => {
  ...
  describe('Advanced Settings Additional Checkboxes', () => {
    ...
    it('toggles immediate evaluation checkbox', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('project.settings.immediateEvaluation')).toBeInTheDocument()
      })

      const checkboxes = screen.getAllByRole('checkbox')
      const evalCheckbox = checkboxes.find((cb) => {
        const label = cb.closest('div')?.textContent
        return label?.includes('project.settings.immediateEvaluation')
      })
    ...
    it('shows strict timer option when time limit is enabled', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('project.settings.strictTimer')).toBeInTheDocument()
      })
    })
