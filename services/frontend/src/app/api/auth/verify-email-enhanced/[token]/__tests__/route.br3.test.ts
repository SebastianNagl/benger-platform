...
function makeRequest(host: string) {
  return new NextRequest(
    new URL(`http://localhost/api/auth/verify-email-enhanced/test-token-abc`),
    {
      method: 'POST',
...
describe('verify-email-enhanced route', () => {
  ...
  it('routes localhost:3000 to localhost:8001', async () => {
    ...
    await POST(makeRequest('localhost:3000'), {
  ...
  it('handles cookie domain for localhost (empty domain)', async () => {
    ...
    const req = new NextRequest(
      new URL('http://localhost/api/auth/verify-email-enhanced/tok'),
      {
        method: 'POST',
