...
jest.mock('recharts', () =>
  require('@/components/reports/view/__mocks__/testUtils').rechartsMock(),
)
  ...
  useI18n: () => ({
    ...
    t: (
      ...
      return base.replace(/\{(\w+)\}/g, (m, name) =>
        v[name] !== undefined ? String(v[name]) : m,
      )
    },
  }),
}))
...
jest.mock('@/lib/api/evaluation-types', () => ({
  getMetricDefinitions: () => ({
    ...
    llm_judge_falloesung_grade_points: {
      display_name: 'Notenpunkte (Falllösung)',
      display_scale: '0-18',
    },
  }),
}))
...
jest.mock('react', () => ({
  ...
  use: (promise: any) =>
    promise && typeof promise.then === 'function' ? mockUseResult : promise,
}))
  ...
  if (overrides.sections) {
    merged.content = {
      ...
      sections: Object.fromEntries(
        Object.entries(base.content.sections).map(([k, v]) => [
          k,
          { ...(v as any), ...(overrides.sections[k] ?? {}) },
        ]),
      ) as any,
    }
  }
...
function mockData(
  report: any = makeReport(),
  snapshot: any = REPORT_SNAPSHOT_FIXTURE,
) {
...
describe('ReportViewerPage', () => {
  ...
  it('renders header, tiles and prose sections from snapshot + content', async () => {
    ...
    expect(screen.getByTestId('section-data')).toHaveTextContent(
      'Der Datensatz umfasst 15 Aufgaben.',
    )
    expect(screen.getByTestId('section-annotations')).toHaveTextContent(
      '224 Abgaben von 36 Teilnehmenden wurden erfasst.',
    )
    expect(screen.getByText('Danke an alle.')).toBeInTheDocument()
    expect(screen.getByTestId('section-generation')).toHaveTextContent(
      '4 Sprachmodellen',
    )
    expect(screen.getAllByTestId('model-chip')).toHaveLength(4)
    expect(screen.getByTestId('section-evaluation')).toBeInTheDocument()
    expect(
      screen.getByText('Die Modelle schlagen den Median.'),
    ).toBeInTheDocument()
    expect(screen.getByText('Fazit hier.')).toBeInTheDocument()
    expect(screen.getByTestId('participants-list')).toBeInTheDocument()
    expect(
      within(screen.getByTestId('participants-list')).getByText('KindAlly'),
    ).toBeInTheDocument()
    expect(screen.getByText('Erstellt mit')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: 'BenGER' })).toHaveAttribute(
      'href',
      '/',
    )
    expect(
      screen.queryByRole('link', { name: 'Bearbeiten' }),
  ...
  it('ranks models by mean with badges computed from the sort', async () => {
    mockData(makeReport(), {
      ...
      series: [...REPORT_SNAPSHOT_FIXTURE.series].reverse(),
    })
    await renderPage()
    ...
    const headers = within(screen.getByTestId('ranking-table-models'))
  ...
  it('removes hidden subjects from the ranking', async () => {
    mockData(
      makeReport({
        sections: {
          evaluation: { charts_config: { hidden_subjects: ['gpt-5.4'] } },
        },
      }),
    )
    await renderPage()
  ...
  it('hides the humans table when show_humans is false', async () => {
    mockData(
      makeReport({
        sections: { evaluation: { charts_config: { show_humans: false } } },
      }),
    )
    await renderPage()
  ...
  it('renders the humans table separately ranked by default', async () => {
    ...
    expect(rows.map((r) => r.getAttribute('data-subject'))).toEqual([
      'annotator:KindAlly',
      'annotator:BraveOtter',
    ])
    expect(within(rows[0]).getByTestId('rank-badge')).toHaveTextContent('1')
  })
  ...
  it('shows the empty evaluation state without a snapshot', async () => {
    ...
    await renderPage()
    expect(
      screen.getByText('Für diesen Bericht liegt noch keine Auswertung vor.'),
    ).toBeInTheDocument()
  ...
  it('shows the draft badge and edit link for a superadmin on an unpublished report', async () => {
    ...
    expect(screen.getByRole('link', { name: 'Bearbeiten' })).toHaveAttribute(
      'href',
      '/projects/project-1/report/edit',
    )
    ...
    mockData(
      makeReport({
        sections: {
          ...
          data: { visible: false },
          annotations: {
            custom_text: 'Eigener Abgabentext',
  ...
  it('keeps the annotations section for custom text even with zero submissions', async () => {
    mockData(
      makeReport({ sections: { annotations: { custom_text: 'Nur Text' } } }),
      {
        ...REPORT_SNAPSHOT_FIXTURE,
        statistics: {
          ...REPORT_SNAPSHOT_FIXTURE.statistics,
          annotation_count: 0,
          participant_count: 0,
        },
      },
  ...
  it('shows an in-layout error card with reload and back link', async () => {
    mockGetReportData
      ...
      .mockResolvedValueOnce({ report: makeReport(), snapshot: null })
    await renderPage()
    ...
    expect(
      within(card).getByRole('link', { name: 'Zurück zu den Berichten' }),
  ...
  it.each([401, 403])(
    'shows the not-public card with a login link on %s',
    async (status) => {
      const err = Object.assign(new Error(`HTTP error! status: ${status}`), {
        response: { status },
      })
      ...
      expect(
        within(card).getByRole('link', { name: 'Anmelden' }),
      ).toHaveAttribute('href', '/login?next=%2Freports%2Freport-1')
      expect(
        within(card).getByRole('link', { name: 'Zurück zu den Berichten' }),
      ).toHaveAttribute('href', '/reports')
      expect(screen.queryByTestId('report-error')).not.toBeInTheDocument()
  ...
  it('renders the loading card inside the layout', () => {
    mockGetReportData.mockReturnValue(new Promise(() => {}))
    render(<ReportViewerPage params={Promise.resolve({ id: 'report-1' })} />)
    expect(screen.getByTestId('report-loading')).toHaveTextContent(
...
describe('classifyLoadError', () => {
  it('detects 401/403 via response.status, status or message text', () => {
    expect(
      classifyLoadError(
        Object.assign(new Error('x'), { response: { status: 403 } }),
      ),
    ).toEqual({ forbidden: true, message: null })
    expect(
      classifyLoadError(Object.assign(new Error('x'), { status: 401 })),
    ).toEqual({ forbidden: true, message: null })
    expect(classifyLoadError(new Error('Unauthenticated'))).toEqual({
      forbidden: true,
      message: null,
    })
    expect(
      classifyLoadError(new Error('HTTP error! status: 403 - Forbidden')),
    ).toEqual({ forbidden: true, message: null })
  })
  ...
  it('keeps other errors with their message', () => {
    ...
    ).toEqual({ forbidden: false, message: 'Boom' })
    expect(classifyLoadError(new Error('HTTP error! status: 404'))).toEqual({
      forbidden: false,
      message: 'HTTP error! status: 404',
    })
    expect(classifyLoadError('nope')).toEqual({
      forbidden: false,
      message: null,
    })
    expect(classifyLoadError(null)).toEqual({ forbidden: false, message: null })
    expect(classifyLoadError(new Error(''))).toEqual({
      forbidden: false,
      message: null,
    })
  })
