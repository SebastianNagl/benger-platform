...
export interface Annotation {
  ...
  reviewed_by?: string
  reviewed_at?: string
  review_result?: 'approved' | 'rejected' | 'fixed'
  review_annotation?: AnnotationResult[]
...
// Server-side timer types (Issue #1205)
...
export interface LabelControl {
  name: string
  type: 'Labels' | 'TextArea' | 'Choices' | 'Rating' | 'Number' | 'Angabe'
  to_name?: string
  required?: boolean
  placeholder?: string
  rows?: number
