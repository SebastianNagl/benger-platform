"""
Notification Service for BenGER (canonical, shared).
...
This module is the single source of truth for ``NotificationService`` and the
``notify_*`` helpers. It used to exist as two divergent copies:

* ``services/api/services/email/notification_service.py`` — the full ORM
  implementation (this code): writes via ``Notification(...)`` + ``db.add``,
  gates the in-app channel on the user's ``UserNotificationPreference``, and
  exposes the recipient-resolution / preference / cleanup surface.
* ``services/workers/notification_service.py`` — a 198-line raw-SQL stub that
  ``INSERT``ed via ``CAST(:type AS notificationtype)`` and did **not** gate
  in_app on preferences (it always inserted a row for every recipient).

Both now re-export from here. **Behavior change for workers**: worker-triggered
notifications now go through the ORM path and therefore *respect the in_app
preference* — a recipient with ``in_app_enabled = False`` for that type no
longer gets an in-app row (previously they always did). The raw-SQL
"bypass the enum NAME" rationale is moot: ``Notification.type`` uses
``values_callable`` so the ORM writes the lowercase enum *value*, identical to
what the old ``CAST`` produced.

Optional dependencies (``email_service``, ``celery_client``, ``email_validation``)
are imported defensively below so the module loads in both the api container
(where all three resolve) and the workers container (where ``celery_client``
and ``email_validation`` are absent — the email-dispatch path degrades to a
logged warning while the in-app commit still succeeds).
"""
...
#
# In the api container `celery_client.get_celery_app` resolves. In the workers
# container there is no `celery_client` module, but `celery` itself is
# installed and a fully-configured app is already bound to `current_app`, so we
# fall back to that — this keeps worker-side email dispatch working (it was
# previously done via a bare `from celery import current_app` in the worker's
# own NotificationService copy). Only when neither is importable do we degrade
# to a raising stub, which the dispatch site catches and logs.
try:
    ...
    try:
        ...
        def get_celery_app():
            raise RuntimeError("no celery app available — email dispatch disabled")
